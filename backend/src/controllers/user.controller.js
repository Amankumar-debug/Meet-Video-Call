...
const register=async (req,res)=>{
    ...
    if(!username||!password||!name){
     ...
     try {
        ...
        await newUser.save();
        return res.status(httpStatus.CREATED).json({message:"user created succesfully"});
     } catch (error) {
...
const addToHistory=async(req,res)=>{
    ...
    try {
       ...
       res.status(httpStatus.CREATED).json({message:"Addesd code to history"})
    } catch (e) {
         re.json({message:`something went wrong${e}`    })
    }
}
